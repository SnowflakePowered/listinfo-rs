...
mod tests {
    ...
    fn test_parse_header() {
        ...
        assert_eq!(
            ...
            Some(&EntryData::Value("Test Description"))
        );
        assert_eq!(
            header.get_unique("version"),
            Some(&EntryData::Value("42069"))
        );
        assert_eq!(
            header.get_unique("author"),
            Some(&EntryData::Value("TestAuthor"))
        );
    }
    ...
    fn test_parse_game() {
        ...
        assert_eq!(game.get_unique("name"), Some(&EntryData::Value("Test")));
        assert_eq!(
            game.get_unique("description"),
            Some(&EntryData::Value("SCPH-101 (Version 4.4 03/24/00 A)"))
        );

        let iter = game.get_iter("rom");
        if let Some(roms) = iter {
            for rom in roms {
                if let EntryData::Node(sub) = rom {
        ...
        if let Some(sample) = iter {
            for sample in sample {
                if let &EntryData::Value(value) = sample {
    ...
    fn test_parse_document() {
        ...
        if let Some(header) = doc.get_entries("clrmamepro") {
            for fragment in header {
                assert_eq!(fragment.get_unique("name"), Some(&EntryData::Value("Test")));
                assert_eq!(
        ...
        if let Some(games) = games {
            for game in games {
                assert_eq!(game.get_unique("name"), Some(&EntryData::Value("psone-44a")));
                assert_eq!(
                    game.get_unique("description"),
                    Some(&EntryData::Value("SCPH-101 (Version 4.4 03/24/00 A)"))
                );
